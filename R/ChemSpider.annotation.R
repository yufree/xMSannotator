...
ChemSpider.annotation <- function(dataA, max.mz.diff = 10, 
    queryadductlist = c("M+H"), xMSannotator.outloc, 
    numnodes = 1, datasources = c("KEGG"), tokenstr = NA, 
    ...
    # 'Molecular.Formula','SMILES','InChI',
    # 'InChIKey',
    ...
    for (adnum in 1:length(queryadductlist)) {
            ...
            if (length(s1) > 2) {
                ...
                repeat {
                  cur.annot.res <- parLapply(cl, s1, 
                    ...
                    datasources, tokenstr, maxhits, 
                    syssleep, adduct_table = adduct_table)
                  if (is(cur.annot.res, "try-error")) {
                    Sys.sleep(10)
                    cur.annot.res <- parLapply(cl, 
                      s1, chspider.batch.annotation.child, 
                      max.mz.diff = max.mz.diff, adductname = adductname, 
                      datasources, tokenstr, maxhits, 
                      syssleep, adduct_table = adduct_table)
        ...
        if (length(res) > 0) {
            ...
            text_resindex <- c(1, 2, 3, 6:9, 10:17, 
                seq(18, 50, 2))
    ...
    if (length(parentres) > 0) {
        ...
        html_resindex <- c(1, 2, 5:16, 17, seq(19, 
            51, 2))
        ...
        # 'Metlin.match.mass', 'Metlin.compound.name',
        # 'CASID', 'KEGG.Compound.ID',
        # 'KEGG.Pathway.name', 'KEGG.Pathway.ID',
        # 'HMDB.ID', 'PubChem.Substance.ID',
        # 'PubChem.Compound.ID', 'ChEBI.ID',
        # 'LIPID.MAPS.ID')
        ...
        cnames <- c("Adduct", "Query.m/z", "Search mass \n tolerance range (+/-)", 
            ...
            "NominalMass", "ALogP", "XLogP", "Structure", 
            "KEGG.Compound.ID", "HMDB", "LipidMAPS", 
            "PubChem", "MassBank", "BioCyc", "SMPDB", 
            "EPA.DSSTox", "EPA.Toxcast", "Pesticide.Common.Names", 
            "ChEMBL", "ChEBI", "NIST.Chem.WebBook", 
        ...
        cnames2 <- cnames[c(1:3, 4, 5:6, 12, 16, 7:9, 
            17:33)]
        html_res <- html_res[, c(1:3, 4, 5:6, 12, 
            16, 7:9, 17:33)]
        colnames(html_res) <- cnames2
        
        fname = paste("ChemSpider_annotation_results", 
            sep = "")
        unlink(fname)
        ...
        if (length(queryadductlist) > 1) {
            ...
            # tolerance range (+/-)', 'Metlin.match.ID',
            # 'Metlin.match.mass', 'Metlin.compound.name',
            # 'CASID', 'KEGG.Compound.ID', 'KEGG.Pathway.ID',
            # 'KEGG.Pathway.name', 'HMDB.ID',
