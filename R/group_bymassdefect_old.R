group_bymassdefect_old <- function(dataA, pct.mz.diff = 10) {
    
    
    mzdefect <- dataA$mz - floor(dataA$mz)
    
    
    # print(mzdefect[1:10]) dataA<-cbind(dataA)
    ...
    mz_groups <- lapply(1:dim(dataA)[1], function(j) {
        ...
        getbind_same <- which(abs(mzdefect - mzdefect[j]) <= 
            ppmb)
    ...
    for (m in 1:length(mz_groups)) {
        ...
        if ((m %in% del_list) == FALSE) {
            for (n in (m + 1):length(mz_groups)) {
                ...
                com1 <- intersect(mz_groups[[m]], 
                  mz_groups[[n]])
                if (length(com1) > 0) {
                  
                  mz_groups[[m]] <- c(mz_groups[[m]], 
                    mz_groups[[n]])
    ...
    for (k in 1:length(mz_groups)) {
        ...
        for (m in 1:length(mz_groups[[k]])) {
            ...
            gid[mz_groups[[k]][m]] <- paste("MDgroup", 
                mz_groups[[k]][1], sep = "")
