Annotationbychemical_IDschild <- function(adduct_index = NA, 
    ...
    if (length(gcur$common) > 0) {
        ...
        mcur <- merge(keggCompMZ[which(keggCompMZ$Adduct %in% 
            adduct_table$Adduct), ], gcur$common, 
            by.x = "mz", by.y = "mz.data.B")
        
        # print(mcur[1,1:4]) print(dataA[1,])
        ...
        if (dim(mcur)[1] > 1) {
            ...
            match_status_mat <- cbind(rownames(h1), 
                match_status)
            
            
            
            colnames(match_status_mat) <- c("mz", 
                "MatchCategory")
            match_status_mat <- as.data.frame(match_status_mat)
            
            mcur_5 <- merge(match_status_mat, mcur_4, 
                by = "mz")
            ...
            cnames1 <- c(cnames1[1], "MatchCategory", 
                cnames[-c(1)])
            mcur_5 <- cbind(mcur_4[1, 1], MatchCategory, 
                mcur_4[, -c(1)])
            mcur_5 <- as.data.frame(mcur_5)
        ...
        if (otherinfo == TRUE) {
            info_mat <- sapply(1:dim(mcur_5)[1], function(j) {
                ...
                b1 <- keggGet(paste("cpd:", mcur_5[j, 
                  1], sep = ""))
                brite_inf <- paste(b1[[1]]$BRITE, 
                  collapse = ";")
                path_inf <- paste(b1[[1]]$PATHWAYS, 
                  collapse = ";")
                otherdb_inf <- paste(b1[[1]]$DBLINKS, 
                  collapse = ";")
                r1 <- c(as.character(mcur_5[j, 1]), 
                  as.character(brite_inf), as.character(path_inf), 
                  as.character(otherdb_inf))
            ...
            colnames(info_mat_1) <- c("chemical_ID", 
                "BriteCategory", "Pathways", "ExternalLinks")
            ...
            if (otherdbs == TRUE) {
                info_mat_2 <- sapply(1:dim(mcur_7)[1], 
                  function(j) {
                      ...
                      if (length(g_ind) > 0) {
                        ...
                        if (length(g_ind) > 1) {
                          for (g in g_ind) {
                            ...
                            lipidmapsID <- c(lipidmapsID, 
                              gsub(b1[g, 2], pattern = "lipidmaps:", 
                                replacement = ""))
                    ...
                    c1 <- c(as.character(mcur_7[j, 
                      1]), hmdbID, lipidmapsID)
                    c1 <- as.data.frame(c1)
                    return(c1)
                  })
