getscore <- function(k, mchemdata, chemids, corthresh, global_cor, 
    mzid, max_diff_rt, level_module_isop_annot, adduct_table, 
    adduct_weights, mass_defect_window = 0.01, mass_defect_mode = "pos") {
    ...
    chem_score <- get_chemscorev1.6.71(chemicalid = chemid, 
        ...
        global_cor, mzid, max_diff_rt = max_diff_rt, level_module_isop_annot = level_module_isop_annot, 
    ...
    if (length(chem_score) > 0) {
        if (chem_score$chemical_score >= (-100)) {
            ...
            chemscoremat <- cbind(chem_score$chemical_score, 
                chem_score$filtdata)
            rm(chem_score)
