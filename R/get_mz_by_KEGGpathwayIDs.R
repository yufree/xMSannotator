...
get_mz_by_KEGGpathwayIDs <-
function(kegg_pathway_ids,outloc=NA,queryadductlist=c("M+H"),max.mz.diff=10,syssleep=0.01){
    ...
    if(length(keggIDs)>0){
        map_res<-get_mz_by_KEGGcompoundIDs(keggIDs,queryadductlist,max.mz.diff,syssleep)
