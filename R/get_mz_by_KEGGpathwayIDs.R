...
get_mz_by_KEGGpathwayIDs <- function(kegg_pathway_ids, 
    outloc = NA, queryadductlist = c("M+H"), max.mz.diff = 10, 
    syssleep = 0.01) {
    ...
    for (kegg_pathway_id in kegg_pathway_ids) {
        ...
        if (length(kegg_comp_list) < 1) {
            ...
            kegg_module_list <- c(kegg_module_list, 
                k1[[1]]$MODULE)
    ...
    if (length(keggIDs) > 0) {
        map_res <- get_mz_by_KEGGcompoundIDs(keggIDs, 
            queryadductlist, max.mz.diff, syssleep)
