multilevelannotationstep2 <- function(outloc1, list_number) {
    ...
    try(load("step1_results.Rda"), silent = TRUE)
    ...
    if (list_number > length(chemids_split)) {
        ...
        # stop(paste('Invalid list number. Must be less
        # than or equal to
        # ',length(chemids_split),sep=''))
        return(0)
    }
    ...
    chem_score <- lapply(chemids_split[[list_number]], 
        function(j) {
            chemid <- chemids[j]
            chemscoremat <- {
            }
            ...
            isp_masses_mz_data <- as.data.frame(isp_masses_mz_data)
            # source('/home/kuppal2/Documents/Projects/xMSannotator//xMSannotator_v1.8.1_debug18.R')
            chem_score <- get_chemscorev1.6.71(chemicalid = chemid, 
                ...
                degree_weights = alldegrees, filter.by, 
                max_isp = max_isp, MplusH.abundance.ratio.check = MplusH.abundance.ratio.check, 
            ...
            if (length(chem_score) > 0) {
                if (chem_score$chemical_score >= (-100)) {
                  ...
                  if (nrow(chem_score$filtdata) > 
                    0) {
                    cur_chem_score <- chem_score$chemical_score
                    chemscoremat <- cbind(cur_chem_score, 
                      chem_score$filtdata)
                  } else {
                    # print(chem_score)
                  }
            ...
            rm("curmchemdata", "isp_masses_mz_data", 
                "mzid_cur")
            return(chemscoremat)
        })  #,mc.cores=num_nodes,mc.preschedule=FALSE)
    
    # stopCluster(cl)
    cur_fname <- paste("chem_score", list_number, 
        "_a.Rda", sep = "")
    ...
    chem_score2 <- chem_score[which(chem_score != 
        "NULL")]
    ...
    rm("curchemscoremat", "mchemdata", "chemids", 
        "adduct_table", "global_cor", "mzid", "max_diff_rt", 
        "isop_res_md", "corthresh", "level_module_isop_annot", 
