get_mz_by_KEGGdrugIDs <- function(keggIDs, queryadductlist = c("M+H"), 
    ...
    cnames <- c("mz", "chemical_ID", "Name", "Formula", 
        ...
        if (is(d2, "try-error")) {
            ...
            if (length(exact_mass) > 0) {
                for (adnum in 1:length(queryadductlist)) {
                  ...
                  mz = ((exact_mass * adductnmol) + 
                    (adductmass))/adductcharge
                  
                  
                  delta_ppm = (max.mz.diff) * (mz/1e+06)
                  min_mz = round((mz - delta_ppm), 
                    5)
                  max_mz = round((mz + delta_ppm), 
                    5)
    ...
    cnames <- c("mz", "KEGGID", "Name", "Formula", 
        "MonoisotopicMass", "Adduct", "AdductMass")
