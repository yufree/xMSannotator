...
#' @param hclustmethod Linkage method for hierarchical clustering e.g.:
#' "complete", "average", "single","ward", "median","centroid","mcquitty".
#' Please see flashClust package for reference.
...
multilevelannotation <-
                 ...
                 pathwaycheckmode = "pm")
        {
                ...
                if (queryadductlist == "all" & mode == "pos") {
                        adduct_names <-
                                adduct_table$Adduct[(adduct_table$Type == "S" &
                                                             adduct_table$Mode == "positive") |
                ...
                if (db_name == "HMDB") {
                        ...
                        hmdbCompMZ$mz <-
                                round(as.numeric(as.character(hmdbCompMZ$mz)), 5)
                        
                        hmdbCompMZ$Name <-
                                gsub(hmdbCompMZ$Name,
                                     pattern = "[\\\"\']",
                                     replacement = "")
                        
                        hmdbbad <-
                                c("HMDB29244", "HMDB29245", "HMDB29246")
                        ...
                        if (db_name == "KEGG") {
                                ...
                                keggCompMZ$mz <-
                                        round(as.numeric(as.character(keggCompMZ$mz)), 5)
                                ...
                                if (db_name == "LipidMaps") {
                                        ...
                                        lipidmapsCompMZ$mz <-
                                                round(as.numeric(
                                                        as.character(lipidmapsCompMZ$mz)
                                                ),
                                                5)
                                        ...
                                        if (db_name == "T3DB") {
                                                ...
                                                print(
                                                        "Dimension of precomputed T3DB m/z database"
                                                )
                                                print(dim(chemCompMZ))
                ...
                if (is.na(adduct_weights) == TRUE) {
                        ...
                        adduct_weights <-
                                as.data.frame(adduct_weights1)
                        colnames(adduct_weights) <-
                                c("Adduct", "Weight")
                        adduct_weights <-
                                as.data.frame(adduct_weights)
                ...
                if (length(check_step1) < 1) {
                        ...
                        mzid <-
                                paste(dataA$mz, dataA$time, sep = "_")
                        ...
                        randmzset <-
                                sample(x = 1:dim(dataA)[1], size = randsetsize)
                        quant1 <-
                        ...
                        l1 <- list.files(outloc)
                        check_levelB <-
                                which(l1 == "xMSannotator_levelB.Rda")
                        ...
                        if (length(check_levelB) < 1) {
                                ...
                                levelB_res_check <-
                                        parLapply(cl, 1:length(uniq_formula), function(j,
                                                                                       uniq_formula,
                                                                                       NOPS_check) {
                                                curformula <- as.character(uniq_formula[j])
                                                return(
                                                        check_golden_rules(curformula, NOPS_check = NOPS_check)
                                                )
                                ...
                                if (length(water_adduct_ind) > 0) {
                                        ...
                                        levelB_res <-
                                                levelB_res[-c(water_adduct_ind), ]
                                        
                                        sind1 <-
                                                seq(1:dim(levelB_res2)[1])
                                        
                                        levelB_res_check3 <-
                                                parLapply(cl, sind1, function(j) {
                                                        adduct <- as.character(levelB_res2$Adduct[j])
                                                        curformula <-
                                                                as.character(levelB_res2$Formula[j])
                                                        
                                                        numoxygens <-
                                                                check_element(curformula, "O")
                                        ...
                                        if (length(which(levelB_res_check4[, 2] == 1)) > 0) {
                                                ...
                                                valid_form <-
                                                        which(
                                                                levelB_res2$Formula %in% as.character(levelB_res_check4[, 1])
                                                        )
                                        }
                        ...
                        #print("here")
                        no_match <-
                                dataA[-which(dataA$mz %in% levelB_res$mz), 1:2]
                        
                        no_match_annot <-
                                matrix(nrow = dim(no_match)[1],
                                       ncol = 8,
                                       "-")
                        ...
                        colnames(no_match_1) <-
                                colnames(annot_chid_res_1)
                        
                        annot_chid_res_1 <-
                                rbind(annot_chid_res_1, no_match_1)
                        
                        last_col_ind <- dim(annot_chid_res_1)[2]
                        
                        annot_chid_res_1 <-
                                annot_chid_res_1[, c(1, 10, 2:8)]
                        ...
                        levels_A <-
                                levels(levelA_res$Module_RTclust)
                        ...
                        mzdefect <-
                                1 * ((levelA_res$mz - floor(levelA_res$mz)))
                        ...
                        if (dim(diffmatB)[1] < dim(dataA)[1]) {
                                ...
                                if (nrow(diffmatC) > 0) {
                                        ...
                                        diffmatC <-
                                                cbind(isop_last, diffmatC)
                                        colnames(diffmatC) <-
                                                colnames(diffmatB)
                                        
                                        
                                        diffmatD <-
                                                rbind(diffmatB[, c(1:10)], diffmatC[, c(1:10)])
                                        rm(levelA_res2)
                                        rm(diffmatC)
                                        rm(diffmatB)
                                } else{
                        ...
                        MD <- diffmatD[, c(2)]
                        levelA_res1 <-
                                cbind(levelA_res1[, c(1:4)], mean_int_vec, MD)
                        ...
                        m1 <-
                                merge(annot_chid_res_1, levelA_res1, by = "mz")
                        ...
                        m2$MatchCategory[which(m2$mz %in% uniquemz)] <-
                                "Unique"
                        
                        munique <-
                                m2[which(m2$MatchCategory == "Unique"), ]
                        ...
                        tmultimatches <-
                                table(m2$chemical_ID, m2$Adduct)
                        smutimatches <- apply(tmultimatches, 1, sum)
                        
                        
                        multiresmat <-
                                m2[order(m2$Module_RTclust, m2$chemical_ID), ]
                        ...
                        tablemz <-
                                table(multiresmat$Module_RTclust, multiresmat$mz)
                        ...
                        tall <-
                                table(multiresmat$chemical_ID, multiresmat$mz)
                        ...
                        tall_multichems <-
                                tall[, which(tall_checkmultichems > 1)]
                        
                        names_tall_multichems <-
                                colnames(tall_multichems)
                        
                        tall_checkmultimz <- apply(tall, 1, sum)
                        
                        tall_multimzperchem <-
                                tall[which(tall_checkmultimz > 1), ]
                        tall_unimzperchem <-
                                tall[which(tall_checkmultimz == 1), ]
                        ...
                        mchemdata$mz <-
                                as.numeric(as.character(mchemdata$mz))
                        mchemdata$time <-
                                as.numeric(as.character(mchemdata$time))
                        
                        #print(mchemdata[1:3,])
                        bad_rows <-
                                which(abs(mchemdata$time - mchemdata$time.y) > 0)
                        ...
                        list_size <-
                                round(length(chemids) / list_winsize)
                        ...
                        #if(FALSE)
                        {
                                ...
                                mchemdata$time <-
                                        round(as.numeric(as.character(mchemdata$time)), 1)
                                mchemdata$MD <-
                                        round(as.numeric(as.character(mchemdata$MD)), 3)
                                mchemdata$mean_int_vec <-
                                        round(as.numeric(
                                                as.character(mchemdata$mean_int_vec)
                                        ), 1)
                                mchemdata$time.y <-
                ...
                if (allsteps == TRUE) {
                        ...
                        lapply(1:num_sets, function(arg1) {
                                cur_fname <- paste(outloc,
                                                   "/stage2/chem_score",
                                                   arg1,
                                                   ".Rda",
                                                   sep = "")
                                check_if_exists <-
                                        suppressWarnings(try(load(cur_fname), silent = TRUE)
                                        )
                ...
                print(
                        "Stage 2 includes clustering results along with simple m/z based database matching"
                )
