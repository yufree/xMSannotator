...
multilevelannotation <- function(dataA, max.mz.diff = 10, 
    ...
    num_sets = 30, allsteps = TRUE, corthresh = 0.7, 
    NOPS_check = TRUE, customIDs = NA, missing.value = NA, 
    hclustmethod = "complete", deepsplit = 2, networktype = "unsigned", 
    minclustsize = 10, module.merge.dissimilarity = NA, 
    filter.by = c("M+H"), redundancy_check = TRUE, 
    min_ions_perchem = 1, biofluid.location = "Blood", 
    origin = "Endogenous", status = "Detected and Quantified", 
    boostIDs = NA, max_isp = 5, MplusH.abundance.ratio.check = FALSE, 
    customDB = NA, HMDBselect = "union", mass_defect_window = 0.01, 
    mass_defect_mode = "pos", dbAllinf = NA, pathwaycheckmode = "pm") {
    options(warn = -1)
    ...
    if (db_name == "HMDB") {
        ...
        hmdbAllinf$Name <- gsub(hmdbAllinf$Name, pattern = "[\\\"']", 
            replacement = "")
        ...
        suppressWarnings(if (is.na(customIDs) == TRUE) {
            if (is.na(biofluid.location) == FALSE & 
                ...
                if (is.na(biofluid.location) == FALSE & 
                  ...
                  if (is.na(biofluid.location) == 
                    TRUE & is.na(origin) == FALSE) {
        ...
        hmdbCompMZ$Name <- gsub(hmdbCompMZ$Name, pattern = "[\\\"']", 
            replacement = "")
        
        hmdbbad <- c("HMDB29244", "HMDB29245", "HMDB29246")
        
        if (length(which(hmdbCompMZ$HMDBID %in% hmdbbad)) > 
            0) {
    ...
    if (length(check_step1) < 1) {
        ...
        if (length(check_levelA) < 1) {
            if (clustmethod == "WGCNA") {
                levelA_res <- get_peak_blocks_modulesvhclust(dataA, 
                ...
                if (clustmethod == "graph") {
                  levelA_res <- get_peak_blocks_graph(dataA, 
                    ...
                    outloc, column.rm.index = NA, 
                    cor.thresh = NA, cormethod = cormethod, 
                    networktype = networktype, num_nodes = num_nodes)
                  ...
                  if (clustmethod == "hclust") {
                    levelA_res <- get_peak_blocks_hclust(dataA, 
                      time_step = time_step, max.rt.diff = max_diff_rt, 
                      outloc, column.rm.index = NA, 
                      cor.thresh = NA, deepsplit = deepsplit, 
        ...
        quant1 <- apply(global_cor[randmzset, ], 1, 
            function(x) {
        ...
        if (length(check_levelB) < 1) {
            ...
            l2 <- parLapply(cl, s1, Annotationbychemical_IDschild, 
                ...
                adduct_table = adduct_table, max.mz.diff = max.mz.diff, 
                outloc = outloc, keggCompMZ = chemCompMZ, 
                otherdbs = FALSE, otherinfo = FALSE)
            ...
            if (length(water_adduct_ind) > 0) {
                ...
                levelB_res_check3 <- parLapply(cl, 
                  sind1, function(j) {
                ...
                if (length(which(levelB_res_check4[, 
                  2] == 1)) > 0) {
                  ...
                  valid_form <- which(levelB_res2$Formula %in% 
                    as.character(levelB_res_check4[, 
                      1]))
                }
        ...
        # print('here')
        no_match <- dataA[-which(dataA$mz %in% levelB_res$mz), 
            1:2]
        
        no_match_annot <- matrix(nrow = dim(no_match)[1], 
            ncol = 8, "-")
        
        no_match_1 <- cbind(no_match[, 1], no_match_annot, 
            no_match[, -c(1)])
        ...
        annot_chid_res_1 <- rbind(annot_chid_res_1, 
            no_match_1)
        
        last_col_ind <- dim(annot_chid_res_1)[2]
        
        annot_chid_res_1 <- annot_chid_res_1[, c(1, 
            10, 2:8)]
        ...
        mean_int_vec <- apply(levelA_res1[, -c(1:4)], 
            1, function(x) {
                mean(x, na.rm = TRUE)
            })
        
        isop_res_md <- cbind(diffmatD[, c(4, 5, 1, 
            3)], mean_int_vec, diffmatD[, c(2)])
        ...
        MD <- diffmatD[, c(2)]
        levelA_res1 <- cbind(levelA_res1[, c(1:4)], 
            mean_int_vec, MD)
        ...
        m1 <- merge(annot_chid_res_1, levelA_res1, 
            by = "mz")
        ...
        multiresmat <- m2[order(m2$Module_RTclust, 
            m2$chemical_ID), ]
        ...
        save(list = c("mchemdata", "chemids", "adduct_table", 
            ...
            "chemids_split", "corthresh", "max.mz.diff", 
            "outloc", "num_sets", "db_name", "num_nodes", 
            "num_sets", "adduct_weights", "alldegrees", 
            "filter.by", "max.rt.diff", "max_isp", 
            "MplusH.abundance.ratio.check", "mass_defect_window", 
            "mass_defect_mode"), file = "step1_results.Rda")
    ...
    if (allsteps == TRUE) {
        ...
        lapply(1:num_sets, function(arg1) {
            ...
            if (is(check_if_exists, "try-error")) {
                suppressWarnings(multilevelannotationstep2(outloc1 = outloc, 
                  list_number = arg1))
        ...
        if (allsteps == TRUE) {
            ...
            suppressWarnings(annotres <- multilevelannotationstep3(outloc, 
            ...
            suppressWarnings(annotresstage4 <- multilevelannotationstep4(outloc, 
                max_diff_rt = max_diff_rt, filter.by = filter.by, 
                adduct_weights = adduct_weights, min_ions_perchem = min_ions_perchem, 
                boostIDs = boostIDs, max_isp = max_isp))
            
            # print('Stage 4 confidence level distribution for
            # unique chemical/metabolite IDs')
            ...
            if (redundancy_check == TRUE) {
                ...
                suppressWarnings(annotresstage5 <- multilevelannotationstep5(outloc, 
                  max_diff_rt = max_diff_rt, filter.by = filter.by, 
                ...
                # print('Stage 5 confidence level distribution for
                # unique chemical/metabolite IDs')
