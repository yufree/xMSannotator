multilevelannotationstep3 <- function(outloc1, adduct_weights = NA, 
    ...
    for (sind in seq(1, num_sets)) {
        cur_fname <- paste("chem_score", sind, ".Rda", 
            sep = "")
    ...
    if (length(which(chemscoremat$chemical_ID %in% 
        hmdbbad)) > 0) {
        chemscoremat <- chemscoremat[-which(chemscoremat$chemical_ID %in% 
            hmdbbad), ]
    }
    
    write.table(chemscoremat, file = "xMSannotator_scoremat_stage2.txt", 
        sep = "\t", row.names = FALSE)
    
    # print('Final chemscoremat')
    # print(dim(chemscoremat))
    otherchems <- mchemdata[which(chemscoremat$mz %in% 
        chemscoremat)]
        ...
        if (is.na(pathwaycheckmode) == FALSE) {
            ...
            for (path_id in pathway_ids) {
                ...
                if (path_id != "-" & path_id != "map01100") {
                  pathway_chemicals <- m2[which(m2[, 
                    2] %in% path_id), 1]  #m2[which(m2[,2]%in%pathwayscurchemical),1]
                  ...
                  # focus molecules not associated with this pathway
                  # (b)
                ...
                for (path_id in pathway_ids) {
                  ...
                  if (path_id != "-") {
                    ...
                    counts = matrix(data = c(num_chems_inpath, 
                      ...
                      num_chems_notinpath, all_notcurpath_numchem), 
                      nrow = 2)
    ...
    chemscoremat$score[which(chemscoremat$chemical_ID %in% 
        boostIDs)] <- max(chemscoremat$score, na.rm = TRUE) * 
        100
    ...
    # write.table(chemscoremat_highconf,file='../Stage3.txt',sep='\t',row.names=FALSE)
    ...
    rm("mchemdata", "chemids", "adduct_table", "global_cor", 
        ...
        "dataA", "corthresh", "max.mz.diff", "outloc", 
        "num_sets", "db_name", "num_nodes", "num_sets", 
        "adduct_weights", "alldegrees", "filter.by")
