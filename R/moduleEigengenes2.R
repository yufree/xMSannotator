moduleEigengenes2 <- function(expr, colors, impute = TRUE, 
    ...
    if (is.null(dim(expr)) || length(dim(expr)) != 
        2) 
    ...
    validColors = colors
    names(PrinComps) = paste(moduleColor.getMEprefix(), 
        modlevels, sep = "")
    ...
    for (i in c(1:length(modlevels))) {
        ...
        pc = try({
            ...
            svd1 = svd(datModule, nu = min(n, p, nPC), 
                nv = min(n, p, nPC))
            if (verbose > 5) 
            ...
            varExpl[c(1:min(n, p, nVarExplained)), 
                i] = apply(veMat^2, 1, mean, na.rm = TRUE)
            svd1$v[, 1]
        }, silent = TRUE)
            ...
            if (subHubs) {
                if (verbose > 0) {
                  ...
                  printFlush(paste(spaces, "     ", 
                    pc, spaces, " ..hub genes will be used instead of principal components."))
                }
                isPC[i] = FALSE
                pc = try({
                  ...
                  isHub[i] = TRUE
                  pcxMat = scaledExpr * matrix(kIM * 
                    alignSign, nrow = nrow(scaledExpr), 
        ...
        if (class(pc) == "try-error") {
            ...
            if (verbose > 0) {
                printFlush(paste(spaces, " ..ME calculation of module", 
                  modulename, "failed with the following error:"))
                printFlush(paste(spaces, "     ", 
                  pc, spaces, " ..the offending module has been removed."))
            }
            ...
            ae = try({
                ...
                if (align == "along average") {
                  ...
                  # return(averExpr)
                  if (cor(averExpr[, i], PrinComps[, 
                    i], use = "p") < 0) 
                    PrinComps[, i] = -PrinComps[, 
                      i]
                }
                0
            }, silent = TRUE)
            if (class(ae) == "try-error") {
                ...
                if (verbose > 0) {
                  printFlush(paste(spaces, " ..Average expression calculation of module", 
                    modulename, "failed with the following error:"))
                  printFlush(paste(spaces, "     ", 
                    ae, spaces, " ..the returned average expression vector will be invalid."))
                }
    ...
    list(eigengenes = PrinComps, averageExpr = averExpr, 
        ...
        validColors = validColors, allOK = allOK, 
        allPC = allPC, isPC = isPC, isHub = isHub, 
