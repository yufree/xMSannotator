get_peak_blocks_modulesvhclust <- function(dataA, 
    simmat, adjacencyfromsimilarity = FALSE, time_step = 3, 
    max.rt.diff = 10, outloc, column.rm.index = NA, 
    cor.thresh = NA, deepsplit = 2, minclustsize = 20, 
    cutheight = 0.2, cormethod = "spearman", networktype = "unsigned", 
    num_nodes = 2, step1log2scale = TRUE) {
    ...
    netclassA = try(blockwiseModules(datExpr = (data_m), 
        checkMissingData = FALSE, blocks = NULL, maxBlockSize = 5000, 
        blockSizePenaltyPower = 5, randomSeed = 12345, 
        loadTOM = FALSE, corType = "pearson", maxPOutliers = 1, 
        quickCor = 0, pearsonFallback = "individual", 
        ...
        reassignThreshold = 1e-06, minCoreKME = 0.5, 
        minCoreKMESize = minclustsize/3, minKMEtoStay = 0.3, 
        mergeCutHeight = cutheight, impute = TRUE, 
    ...
    if (do_stepwise == 1) {
        ...
        if (adjacencyfromsimilarity == FALSE) {
            ...
            powers = c(c(1:10), seq(from = 12, to = 20, 
                by = 2))
            sft = try(pickSoftThreshold(data = data_m, 
        ...
        dup_names <- which(duplicated(rnames_simmat) == 
            TRUE)
        
        if (length(dup_names) > 0) {
            
            ADJdataOne <- ADJdataOne[-c(dup_names), 
                -c(dup_names)]
        }
        ...
        # if(FALSE)
        {
            ...
            # if(FALSE)
            {
                ...
                if (length(l2colors) > 1) {
                  ...
                  m2 = try(mergeCloseModules(data_m, 
                    colors = colorhdataOne2, cutHeight = cutheight), 
                    silent = TRUE)
    ...
    diffmatB <- lapply(1:length(mod_names), function(i) {
        ...
        groupB <- group_by_rt_hist(subdata, time_step, 
