...
minclustsize=20,cutheight=0.2,cormethod="spearman",networktype = "unsigned",num_nodes=2,step1log2scale=TRUE)
{
...
do_stepwise=0 
   ...
   if(is(sft,"try-error")){
    ...
    if(is.na(power_val)==TRUE){
        ...
        netclassA =try(blockwiseModules(datExpr=(data_m), checkMissingData = FALSE, blocks = NULL, maxBlockSize = 5000,
        blockSizePenaltyPower = 5, randomSeed = 12345, loadTOM = FALSE,
