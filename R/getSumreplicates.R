getSumreplicates <- function(curdata, alignment.tool, 
    numreplicates, numcluster, rep.max.missing.thresh, 
    summary.method = "mean", summary.na.replacement = "zeros", 
    missing.val = 0) {
    ...
    for (samp in seq(1, (numsamp), numreplicates)) {
        ...
        if (dim(curdata[, c(i:j)])[1] > 0) {
            sub_samp_list[[sampcount]] = curdata[, 
                c(i:j)]
        }
    ...
    colnames_data <- colnames_data[seq(1, (numsamp), 
        numreplicates)]
