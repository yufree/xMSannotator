group_metabs_prev <- function(dataA, max.rt.diff = 10, 
            ...
            if ((m %in% cor_group_list) == FALSE) {
                for (n in (m + 1):length(cor_groups)) {
                  ...
                  if (length(com1) > 0) {
                    ...
                    cor_group_list <- c(cor_group_list, 
                      n)
                    cor_groups[[n]][[1]] <- c(0)
                  }
                }
    ...
    # group_labels_level1<-names() Step 1 Group
    # features by m/z
    mz_groups <- lapply(1:length(group_labels_level1), 
        function(j) {
            commat = {
            }
            diffmz = new("list")
            
            cur_group <- diffmat[which(diffmat$levelAnum == 
                group_labels_level1[j]), ]
            ...
            for (i in 1:dim(cur_group)[1]) {
                getbind_same <- which(abs(diffmat$time - 
                  cur_group$time[i]) <= max.rt.diff & 
                  diffmat$levelAnum == group_labels_level1[j])
