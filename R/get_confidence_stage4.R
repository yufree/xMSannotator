get_confidence_stage4 <- function(curdata, max_diff_rt, 
    ...
    adduct_monoisot <- data.frame(cbind("M", 1, 1, 
        0, "-", "S"))
    ...
    temp1 <- curdata
    temp1$Module_RTclust <- gsub(temp1$Module_RTclust, 
        pattern = "(_[0-9]*)", replacement = "")
    ...
    if (length(unique(curdata$Adduct)) < 2 && curdata$score < 
        ...
        if (length(unique(curdata$Adduct)) < 2 && 
            length(which(cur_adducts %in% filter.by)) > 
                0) {
    ...
    if (length(unique(curdata$Adduct)) < 2 && nrow(curdata) > 
        ...
        if (length(which(cur_adducts %in% filter.by)) < 
            ...
            curdata <- curdata[which(cur_adducts %in% 
                filter.by), ]
            score_level <- "2"
    ...
    if (delta_rt > max_diff_rt) {
        ...
        groupB <- group_by_rt(curdata, time_step = 3, 
            max.rt.diff = max_diff_rt, groupnum = "")
        
        
        groupB <- groupB[order(groupB$mz), ]
        
        cname1 <- paste(groupB$mz, groupB$time, sep = "_")
        if (length(which(duplicated(cname1) == TRUE)) > 
            0) {
        ...
        if (length(which(adduct_weights[, 1] %in% 
            cur_adducts)) > 0 && curdata$score[1] > 
            0.1) {
            if (is.na(filter.by) == TRUE) {
                ...
                if (length(which(cur_adducts %in% 
                  filter.by)) > 0) {
                  good_mod <- curdata$Module_RTclust[which(curdata$Adduct %in% 
                    filter.by)]
                  curdata <- curdata[which(curdata$Module_RTclust %in% 
                  ...
                  if (length(which(t1 > 1)) < 1) {
                    if (curdata$score[1] > 0 && nrow(curdata) > 
                      1 && length(unique(curdata$Adduct)) > 
                      1 && delta_rt < max_diff_rt && 
                      length(which(curdata$Adduct %in% 
                        filter.by)) > 0) {
                      chemscoremat_conf_levels <- "High"
                    } else {
        ...
        module_clust <- paste(curdata$module_num, 
            curdata$Module_RTclust, sep = "")
        curdata$Module_RTclust <- module_clust
    ...
    # print('OVER HERE') else
    {
        ...
        if (length(module_names) > 1 && curdata$score < 
            ...
            if (length(module_names) > 1 && curdata$score > 
                10 && length(which(cur_adducts %in% 
                filter.by)) > 0) {
            ...
            if (nrow(curdata) < 2) {
                ...
                if (min_nummol > 1) {
                  ...
                  if (length(check1) > 0) {
                    ...
                    # isotope based check for +1 and +2 to make sure
                    # that the abundant form is present
                    ...
                    if (FALSE) {
                      check2 <- gregexpr(text = cur_adducts_with_isotopes, 
                        pattern = "(_\\[(\\+|\\-)[0-9]*\\])")  #M+H_2
                      curformula <- gsub(curformula, 
                        pattern = "Sr", replacement = "")
                      curformula <- gsub(curformula, 
                        pattern = "Sn", replacement = "")
                      curformula <- gsub(curformula, 
                        pattern = "Se", replacement = "")
                      curformula <- gsub(curformula, 
                        pattern = "Sc", replacement = "")
                      curformula <- gsub(curformula, 
                        pattern = "Sm", replacement = "")
    ...
    formula_vec <- curdata$Formula
    curformula <- as.character(formula_vec[1])
    curformula <- gsub(curformula, pattern = "Ca", 
        replacement = "")
    curformula <- gsub(curformula, pattern = "Cl", 
        replacement = "")
    curformula <- gsub(curformula, pattern = "Cd", 
        replacement = "")
    curformula <- gsub(curformula, pattern = "Cr", 
        replacement = "")
        ...
        if (length(unique(curdata$Adduct)) < 2 && 
            ...
            if (length(unique(curdata$Adduct)) < 2 && 
                ...
                if (length(unique(curdata$Adduct)) < 
                  2 && curdata$score < 10 && length(which(cur_adducts %in% 
                  filter.by)) > 0) {
                  chemscoremat_conf_levels <- "Low"
                  # return(chemscoremat_conf_levels)
                }
    ...
    if (length(which(is.na(final_res$score_level) == 
        TRUE)) > 0) {
        final_res$score_level <- replace(final_res$score_level, 
            which(is.na(final_res$score_level) == 
                TRUE), 0)
    }
    ...
    num_good_adducts <- length(which(uniq_adducts %in% 
        filter.by))
    
    # final_res$score<-(as.numeric(as.factor(final_res$score_level))^num_uniq_adducts)+0.1
    
    # print(final_res$score) print(num_uniq_adducts)
    # print(delta_rt) print(length(module_names))
    ...
    # print('delta') print(delta_rt)
    # if(final_res$chemical_ID=='C00073') {
    
    # print(final_res) stop('Pyridoxine')
    # stop('Methionine') }
