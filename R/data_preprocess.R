data_preprocess <- function(Xmat = NA, Ymat = NA, 
    feature_table_file, parentoutput_dir, class_labels_file, 
    num_replicates = 3, feat.filt.thresh = NA, summarize.replicates = TRUE, 
    summary.method = "mean", all.missing.thresh = 0.5, 
    group.missing.thresh = 0.7, log2transform = TRUE, 
    medcenter = TRUE, znormtransform = FALSE, quantile_norm = TRUE, 
    lowess_norm = FALSE, madscaling = FALSE, missing.val = 0, 
    samplermindex = NA, rep.max.missing.thresh = 0.5, 
    ...
    if (is.na(missing.val) == FALSE) {
        ...
        data_matrix <- replace(as.matrix(data_matrix), 
            which(data_matrix == missing.val), NA)
    }
    ...
    if (is.na(feat.filt.thresh) == FALSE) {
        if (num_replicates > 1) {
            ...
            feat.eval.result = evaluate.Features(cbind(data_matrix[, 
                c(1:2)], data_matrix), num_replicates, 
            ...
            feat.eval.result.mat = cbind(data_matrix[, 
                c(1:2)], feat.eval.result)
    ...
    if (summarize.replicates == TRUE) {
        if (num_replicates > 1) {
            ...
            data_m <- getSumreplicates(data_matrix, 
                alignment.tool = "apLCMS", numreplicates = num_replicates, 
                numcluster = 10, rep.max.missing.thresh = rep.max.missing.thresh, 
            ...
            data_m_prenorm <- cbind(data_matrix[, 
                c(1:2)], data_m)
    ...
    if (is.na(class_labels_file) == FALSE) {
        ...
        for (c in 1:length(class_labels_levels)) {
            if (c > 1) {
                data_matrix <- cbind(data_matrix, 
                  data_subjects[, which(classlabels[, 
        ...
        if (is.na(Ymat) == TRUE) {
            ...
            classlabels <- Ymat
            classlabels <- as.data.frame(classlabels)
            colnames(classlabels) <- c("SampleID", 
                "Class")
            f1 <- table(classlabels$SampleID)
            ...
            for (c in 1:length(class_labels_levels)) {
                ...
                ordered_labels <- c(ordered_labels, 
                  as.character(classlabels[classlabels_index, 
                    2]))
                num_samps_group[[c]] <- length(classlabels_index)
    ...
    if (is.na(all.missing.thresh) == FALSE) {
        ...
        if (length(total_good_metabs) > 0) {
            ...
            # print(paste('Dimension of data matrix after
            # overall ',all.missing.thresh,'% signal threshold
            # filtering',sep='')) print(paste('Dimension of
            # data matrix after using overall
            # ',100*all.missing.thresh, '% signal criteria for
            # filtering:'),sep='') print(dim(data_matrix))
        } else {
    ...
    # print(dim(data_m)) Step 3) Remove features if
    # signal is not detected in at least 70% of
    # samples in either of the groups
    ...
    if (is.na(group.missing.thresh) == FALSE) {
        ...
        if (length(class_labels_levels) == 2) {
            ...
            for (metab_num in 1:dim(data_matrix)[1]) {
                ...
                if ((num_sigsA >= sig_thresh_groupA) || 
                  (num_sigsB >= sig_thresh_groupB)) {
                  clean_metabs <- c(clean_metabs, 
                    metab_num)
                }
                  ...
                  if (is.na(missing.val) == FALSE) {
                    ...
                    num_sigsC <- length(which(data_m[metab_num, 
                      (num_samps_group[[1]] + num_samps_group[[2]] + 
                        1):(num_samps_group[[1]] + 
                        num_samps_group[[2]] + num_samps_group[[3]])] > 
                      missing.val))
                  } else {
        ...
        if (length(clean_metabs) > 0) {
            ...
            # print(paste('Dimension of data matrix after
            # using group-wise ',100*group.missing.thresh, '%
    ...
    # print('before') print(data_m[1:10,1:10]) Step 4)
    ...
    # print('after') print(data_m[1:10,1:10]) Use this
    # if first column is gene/metabolite name for
    # apLCMS: data_matrix_temp<-data_matrix[,c(1:2)]
    data_matrix <- cbind(data_matrix[, c(1:2)], data_m)
    ...
    # print('num rows is ') print(num_rows) for
    # apLCMS:
    ...
    write.table(mzid_mzrt, file = "mzid_mzrt.txt", 
        sep = "\t", row.names = FALSE)
    ...
    write.table(data_matrix, file = filename, sep = "\t", 
        row.names = FALSE)
    data_matrix_prescaling <- cbind(data_matrix[, 
        c(1:2)], data_m_prescaling)
