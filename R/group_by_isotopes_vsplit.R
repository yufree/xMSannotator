group_by_isotopes_vsplit <- function(dataA, step.mz.diff = 1, 
    mdgnum = -1) {
    diffmatB <- {
    }
    ...
    time_cor_groups <- sapply(list(myData1 = dataA), 
        function(x) split(x, cut(mzdefect, breaks = seq(0, 
            1000, time_step))))
    ...
    for (gnum in 1:length(time_cor_groups)) {
        ...
        if (length(time_cor_groups[[gnum]]) > 0) {
            ...
            ISgroup <- paste("ISgroup", mdgnum, gnum, 
                sep = "_")
