group_by_rt_hist <- function(mchemicaldata, time_step = 1, 
    ...
    # if(diff1>max_diff_rt)
    {
        ...
        h1 <- try(hist(mchemicaldata$time, breaks = seq(min(mchemicaldata$time) - 
            max_diff_rt, to = (max_diff_rt + max(mchemicaldata$time)), 
            by = hist_inc), plot = FALSE), silent = TRUE)
        
        if (is(h1, "try-error")) {
            
            h1 <- try(hist(mchemicaldata$time, breaks = seq(min(mchemicaldata$time), 
                to = (max(mchemicaldata$time)), by = bw_i), 
        ...
        clusterlabels3 <- replace(clusterlabels3, 
            which(is.na(clusterlabels3) == TRUE), 
            0)
