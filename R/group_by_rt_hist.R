...
# 	h1<-hist(mchemicaldata$time,breaks=seq(min(mchemicaldata$time),to=(max_diff_rt),by=hist_inc))
	  ...
	  h1<-try(hist(mchemicaldata$time,breaks=seq(min(mchemicaldata$time)-max_diff_rt,to=(max_diff_rt+max(mchemicaldata$time)),by=hist_inc),plot=FALSE),silent=TRUE)

	  if (is(h1, "try-error")){

	  h1<-try(hist(mchemicaldata$time,breaks=seq(min(mchemicaldata$time),to=(max(mchemicaldata$time)),by=bw_i),plot=FALSE),silent=TRUE)
