group_by_rt_density <- function(subdata, time_step = 1, max.rt.diff = 5, 
    groupnum) {
    ...
    time_cor_groups <- sapply(list(myData1 = subdata), function(x) split(x, 
        cut(subdata$time, breaks = d1$x)))
