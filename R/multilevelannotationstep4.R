multilevelannotationstep4 <- function(outloc, max.mz.diff = 5, 
    ...
    curated_res <- merge(chemscoremat_conf_levels, chemscoremat_highconf, 
        by = "chemical_ID")
    ...
    curated_res <- cbind(curated_res[, 1:8], delta_ppm, curated_res[, 
        9:dim(curated_res)[2]])
    ...
    if (is.na(boostIDs) == FALSE) {
        ...
        if (length(cnames_boost) > 1) {
            ...
            ghilicpos <- getVenn(curated_res_mzrt, name_a = "exp", 
                validated_mzrt, name_b = "boost", mz.thresh = max.mz.diff, 
                time.thresh = max_diff_rt, alignment.tool = NA, 
                xMSanalyzer.outloc = getwd(), use.unique.mz = FALSE, 
                plotvenn = FALSE)
            ...
            curated_res$score[overlap_mz_time_id] <- curated_res$score[overlap_mz_time_id] * 
                100
            ...
            if (length(good_ind) > 0) {
                ...
                curated_res$score[good_ind] <- curated_res$score[good_ind] * 
                  100
            }
