get_peak_blocks_hclust <- function(dataA, time_step = 1, 
    max.rt.diff = 5, outloc, column.rm.index = NA, 
    cor.thresh = NA, deepsplit = 4, minclustsize = 2, 
    cutheight = 0.2, cormethod = "spearman") {
    ...
    # if(FALSE)
    {
        hierTOMCormat = hclust(as.dist(dissTOMCormat), 
            method = "complete")
        par(mfrow = c(1, 1))
        pdf("plot.pdf")
        plot(hierTOMCormat, labels = F, main = "Dendrogram")
        
        # m1 <- cutree(hierTOMCormat,
        # h=max(hierTOMCormat$height)/2)
        
        
        m1 = cutreeDynamic(hierTOMCormat, distM = dissTOMCormat, 
            deepSplit = 4, minClusterSize = 1, pamRespectsDendro = TRUE)
        ...
        if (FALSE) {
            colorhdataOne = cutreeDynamic(hierTOMCormat, 
                distM = dissTOMCormat, deepSplit = deepsplit, 
                cutHeight = cutheight, pamRespectsDendro = TRUE)
        ...
        if (FALSE) {
            # colorhdataOne2=cutreeDynamic(hierTOMCormat,distM=
            # dissTOMCormat,deepSplit=1, minClusterSize=2,
            # pamRespectsDendro = FALSE)
            
            colorhdataOne2 = cutreeHybrid(hierTOMCormat, 
                distM = dissTOMCormat, deepSplit = deepsplit, 
                minClusterSize = minclustsize, pamRespectsDendro = TRUE, 
                pamStage = TRUE)
            ...
            if (length(l2colors) > 1) {
                ...
                m2 = mergeCloseModules(data_m, colors = colorhdataOne2$labels, 
                  cutHeight = cutheight)
    ...
    if (FALSE) {
        # hr <-
        # hclust(as.dist(1-cor(t(good_metabs[,-c(1:2)]),use='pairwise.complete.obs')))
        # #metabolites
        hc <- hclust(as.dist(dissTOMCormat))  #,use='pairwise.complete.obs') #samples
        # h73<-heatmap.2(as.matrix(good_metabs[,-c(1:2)]),
        # Rowv=NULL, Colv=as.dendrogram(hc),
        # col=topo.colors(256), scale='none',key=TRUE,
        # symkey=FALSE, density.info='none', trace='none',
        # cexRow=0.5, cexCol=0.85,xlab='',ylab='',
        # main='',ColSideColors=patientcolors)
        
        # mycl_samples <- cutree(hc, h=max(hc$height)/2)
        # mycl_metabs <- cutree(hr, h=max(hr$height)/2)
        
        
        mycl_metabs = cutreeDynamic(hc, distM = dissTOMCormat, 
            deepSplit = 4, minClusterSize = 5, pamRespectsDendro = TRUE)
    }
    ...
    for (i in 1:length(mod_names)) {
        ...
        groupB <- group_by_rt(subdata, time_step, 
            max.rt.diff = max.rt.diff, groupnum = groupA_num)
