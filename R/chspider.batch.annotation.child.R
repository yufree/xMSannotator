chspider.batch.annotation.child <- function(mz.val, 
    max.mz.diff, adductname, datasources = c("KEGG"), 
    tokenstr, maxhits = 30, syssleep = 10, adduct_table) {
    ...
    if (is(csids, "try-error")) {
        ...
        if (is(csids, "try-error")) {
            ...
            # WikiPathways, DrugBank, Comparative
            # Toxicogenomics Database, ACToR: Aggregated
            ...
            if (length(csids) > 0) {
                ...
                for (id_ind in 1:maxhits) {
                  ...
                  for (i in slotnames) {
                    if (i == "CommonName") {
                      ...
                      if (i == "MF") {
                        ...
                        if (i == "SMILES") {
                          if (length(info@SMILES) > 
                            0) {
                            SMILES = info@SMILES
                          }
                        } else {
                          if (i == "InChI") {
                            if (length(info@InChI) > 
                              0) {
                              InChI = info@InChI
                            }
                          } else {
                  ...
                  img_url <- paste("<img src=\"", 
                    fname, "\" alt=", csid, " width=100 height=100>", 
                    sep = "")
                  ...
                  # if(FALSE)
                  {
                    ...
                    annot_res <- rbind(annot_res, 
                      c(mz.val, delta_ppm, info@CSID, 
                        urllink1, urllink2, CommonName, 
                        MF, SMILES, InChI, InChIKey, 
                        AverageMass, MolecularWeight, 
                        MonoisotopicMass, NominalMass, 
                        ALogP, XLogP, img_url, ID.KEGG, 
                        URL.KEGG, ID.HMDB, URL.HMDB, 
                        ID.LipidMAPS, URL.LipidMAPS, 
                        ID.PubChem, URL.PubChem, ID.MassBank, 
                        URL.MassBank, ID.BioCyc, URL.BioCyc, 
                        ID.SMPDB, URL.SMPDB, ID.EPA.DSSTox, 
                        URL.EPA.DSSTox, ID.EPA.Toxcast, 
                        URL.EPA.Toxcast, ID.Pesticide.Common.Names, 
                        URL.Pesticide.Common.Names, 
                        ID.ChEMBL, URL.ChEMBL, ID.ChEBI, 
                        URL.ChEBI, ID.NIST.Chem.WebBook, 
                        URL.NIST.Chem.WebBook, ID.WikiPathways, 
                        URL.WikiPathways, ID.DrugBank, 
                        URL.DrugBank, ID.CTD, URL.CTD, 
                        ID.ACToR, URL.ACToR))
                    countid <- countid + 1
                    if (countid%%50 > 0) {
                      Sys.sleep(syssleep)
