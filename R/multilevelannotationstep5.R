multilevelannotationstep5 <- function(outloc, max_diff_rt, 
    adduct_weights = NA, filter.by = NA, min_ions_perchem = 1, 
    boostIDs = NA, max_isp = 5, dbAllinf = NA, db_name = "HMDB") {
    ...
    outloc2 <- outloc
    # chemscoremat_highconf<-read.delim('Stage3.txt',sep='\t',header=TRUE)
    ...
    chemscoremat <- chemscoremat_highconf[which(chemscoremat_highconf$score >= 
        scorethresh), ]
    ...
    dunique <- chemscoremat[which(chemscoremat$mz %in% 
        mzunique), ]
    ...
    for (mind in 1:length(mzdup)) {
        ...
        dmultsub <- chemscoremat[which(chemscoremat$mz %in% 
            mznum), ]
        ...
        for (com_indval in 1:length(com_ind)) {
            ...
            if (com_indval %in% good_ind == FALSE) {
                # print(com_indval)
                dmat_com <- chemscoremat[which(chemscoremat$chemical_ID %in% 
                  dmultsub$chemical_ID[com_indval]), 
                  ]
                
                scoreval <- ((dim(dmat_com)[1]) - 
                  1) * dmat_com$score[1]/(dim(dmat_com)[1])
                
                scorevec <- c(rep(scoreval, length(which(chemscoremat$chemical_ID %in% 
                  dmultsub$chemical_ID[com_indval]))))
                
                if (length(scorevec) < length(which(chemscoremat$chemical_ID %in% 
                  dmultsub$chemical_ID[com_indval]))) {
                  break
                }
                
                chemscoremat$score[which(chemscoremat$chemical_ID %in% 
    ...
    good_ind <- which(chemscoremat_unique$score >= 
        scorethresh)
    ...
    chemscoremat_unique_highconf$MatchCategory <- gsub(as.character(chemscoremat_unique_highconf$MatchCategory), 
        pattern = "Multiple", replacement = "Unique")
    chemscoremat_unique_highconf$MatchCategory[which(chemscoremat_unique_highconf$mz %in% 
        multi_mz)] <- "Multiple"
    ...
    curated_res <- curated_res[order(curated_res$Confidence, 
        curated_res$chemical_ID, curated_res$score, 
        curated_res$Adduct, decreasing = TRUE), ]
    ...
    HTMLInitFile(filename = fname, Title = "Stage 5 annotation results", 
        outdir = outloc)
    fname = paste(outloc, "/Stage5_annotation_results.html", 
        sep = "")
