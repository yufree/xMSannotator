Metlin.annotation <- function(dataA, max.mz.diff = 10, 
    ...
    adductlist = c(1.00727, 22.989171, 38.963171, 
        -35.012729, -17.002729, 0.0227, 7.01597, 18.033871, 
        33.033471, 42.033871, 44.971171, 64.015771, 
        1.00727, 11.998247, 22.989171, 1.00727, 8.33459, 
        15.6618, -19.01839, -1.00727, 18.998371, 20.974671, 
        34.969371, 36.948571, 44.998171, 59.013871, 
        78.918885, -1.00727, -1.00727)
    alladducts <- c("M+H", "M+Na", "M+K", "M+H-2H2O", 
        ...
        "M+K-2H", "M+FA-H", "M+CH3COO", "M+Br", "M-2H", 
        "M-3H")
    ...
    for (adnum in 1:length(queryadductlist)) {
        ...
        for (mzind in 1:length(mzlist)) {
            ...
            url <- paste(url, suburl, "&adduct[]=", 
                adductname, "&tolunits=ppm&tolerance=", 
                max.mz.diff, sep = "")
                  ...
                  for (m2 in 1:length(metlin_res_m1)) {
                    ...
                    if (length(cur_res) > 1) {
                      ...
                      temp_res <- rbind(temp_res, 
                        c(mzlist[mzind], html_link, 
                          cur_res$molid, cur_res$mass, 
    ...
    if (length(parentres) > 0) {
        ...
        fname = paste("Metlin_annotation_results", 
            sep = "")
        unlink(fname)
